...
public class ConcurrentCommandBuilder {
	...
	private static ConcurrentFilter determineFinalFilter(String command) {
		...
		// find the last pipe and last redirect and check if background signal exists.
		int lastPipeIdx = command.lastIndexOf(ConcurrentREPL.PIPE);
		int lastRedirectIdx = command.lastIndexOf(ConcurrentREPL.REDIRECT);
		int lastBackGroundIdx = command.lastIndexOf(ConcurrentREPL.BACKGROUND);
		
		if (lastBackGroundIdx != -1) {
			ConcurrentREPL.background = true;
			command = command.substring(0,lastBackGroundIdx-1);
		}else {
			ConcurrentREPL.background = false;
		}
